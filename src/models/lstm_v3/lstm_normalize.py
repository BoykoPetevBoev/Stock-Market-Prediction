...
def calclate_sort_term_trend(data):
    ...
    data.loc[(data['Close'] > data['MA20']) & (data['MA10'] > data['MA20']), 'Short_Term_Trend'] = UPWARD
    data.loc[(data['Close'] < data['MA20']) & (data['MA10'] < data['MA20']), 'Short_Term_Trend'] = DOWNWARD
    return data


def calclate_long_term_trend(data):
    data['Long_Term_Trend'] = NEUTRAL
    data.loc[(data['Close'] > data['MA100']) & (data['MA50'] > data['MA100']), 'Long_Term_Trend'] = UPWARD
    data.loc[(data['Close'] < data['MA100']) & (data['MA50'] < data['MA100']), 'Long_Term_Trend'] = DOWNWARD
    return data
...
def prepare_sequences(data: pd.DataFrame, target):
    ...
    # target = data['Short_Term_Trend'].to_numpy()
